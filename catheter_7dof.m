...
% Last Modified by GUIDE v2.5 27-Sep-2018 18:22:47
...
for hh = 1:2
    hold(hh_arr{hh},'on');
    plot3(X(ii_arr(hh):end),Y(ii_arr(hh):end),Z(ii_arr(hh):end),'-','color',color_arr(rr,:),'linewidth',2,'parent',hh_arr{hh}); % plot catheter
    plot3(xh,yh,zh,'color',color_arr(rr,:),'linewidth',1,'parent',hh_arr{hh}); % plot helix
end
plot(x_pks,y_pks,'.','color',color_arr(rr,:),'markersize',14,'parent',handles.axes3);
