...
pdt_txt_arr = {
    ...
    'd_{avg}',                  '\Delta\alpha_{avg}',...
    'd_{std}',                  '\Delta\alpha_{std}',...
    'd_{avg,odd}-d_{avg,even}', '\Delta\alpha_{avg,odd}-\Delta\alpha_{avg,even}',...
    'd_{max}-d_{min}',          '\Delta\alpha_{max}-\Delta\alpha_{min}',...
    '\alpha_{std}',             '\alpha_{max}-\alpha_{min}',...
    };
...
for ii = 1:size(X_ARR,1)
    for jj = 1:size(X_ARR,2)
        ...
        alpha(alpha<0) = alpha(alpha<0) + 2*pi; % wrap angles to [0,2pi] 
        ...
        predictor(:,nn) = [...
            mean(d),                    mean(dalpha),...
            std(d),                     std(dalpha),...
            mean(d_odd)-mean(d_even),   mean(dalpha_odd)-mean(dalpha_even),...
            max(d)-min(d),              max(dalpha)-min(dalpha),...
            std(alpha),                 max(alpha)-min(alpha)];
