...
n_arr = 1; % number of predictors to run in each simulation (nn loop)
...
if TGL_shuffle
    ...
    predictor_org = temp(1:size(PDT,1),:);  % save original predictor array
    response_org = temp(size(PDT,1)+1:end,:);    % save original response array
else
    predictor_org = PDT;
    response_org = RSP;
end
...
for nn = 1:length(n_arr)
    ...
    parfor kk = 1:size(ind_arr,1)
        ...
        %% load predictors
        predictor = predictor_org(ind_arr(kk,:),:); % load predictors
