...
pos_pdt = [0.275,0.6,0.1,0.3];
pos_3d = [0.4,0,0.6,1];
pos_rsp = [0.275,0.1,0.1,0.3];
fsz = 20;
msz = 20;
...
for ii = 102%ind_list
    ...
    for mm = 1:2
        ...
        hc(mm) = plot3(X,Y,Z,'color',cmap(mm,:),'linewidth',6);
        ...
        u = M(1,:); v = M(2,:); w = M(3,:);
        hq = quiver3(xq,yq,zq,u,v,w,'filled','color',cmap(mm,:),'linewidth',2,'maxheadsize',1);
    end
    legend(hc,'ground truth','NN output','location','southwest','orientation','horizontal','fontsize',fsz);
    axis equal;
    axis tight;
    view([-37.5+90,30]);
    grid on;
